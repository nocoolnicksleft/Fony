...
#define LCD_PINSEL  PINSEL1
#define LCD_PINSEL_ON (0xAA << 2)
#define LCD_PINSEL_OFF (0xFF << 2)
#define LCD_SPI_CCR S1SPCCR
#define LCD_CR S1SPCR
#define LCD_SPI_DIVIDER_VALUE 0x000000006



void WriteSpiCommand(unsigned int data)
{
    while((S1SPSR & S0SPSR_SPIF) == 0);                  // if still busy wait here

    PINSEL1 &= ~LCD_PINSEL_OFF; // SPI AUS
    ...
    PINSEL1 |= LCD_PINSEL_ON; // SPI EIN

    S1SPCCR  = LCD_SPI_DIVIDER_VALUE; // Clock Divider, FOR LPC2294: min 0x8, FOR LPC2148: min 0x0, even only!!
    S1SPCR   = (BIT5); // Master mode
...
void WriteSpiData(unsigned int data)
{
    ...
    PINSEL1 |= (0xAA << 2); // SPI EIN

    S1SPCCR  = 0x000000006; // Clock Divider, FOR LPC2294: min 0x8, FOR LPC2148: min 0x0, even only!!
    S1SPCR   = (BIT5); // Master mode
...
void LCDWriteBitmap(int x, int y, int width, int height, unsigned char * data) {
...
// Author: James P Lynch August 30, 2007
// *************************************************************************************************
void LCDPutStr(char *pString, int y, int x, int Size, int fColor, int bColor) {
...
void LCDTestPattern()
{
 // ***************************************************************
  ...
  LCDSetRect( 0, 0, 16, 131, FILL, WHITE);
  LCDSetRect(17, 0, 33, 131, FILL, YELLOW);
  LCDSetRect(34, 0, 56, 131, FILL, CYAN);
  LCDSetRect(51, 0, 67, 131, FILL, GREEN);

  LCDSetRect(68, 0, 84,131, FILL, MAGENTA);
  LCDSetRect(85, 0,101,131, FILL, RED);
  LCDSetRect(102,0,118,131, FILL, BLUE);
  LCDSetRect(119,0,131,131, FILL, BLACK);

  LCDPutStr("YUKI-TV", 40, 80, LARGE, WHITE, BLACK);
}
